...
```{r}
prep <- prep_word2vec(tmp, destination = 'noticias_w2v.txt', bundle_ngrams = 3)
```
...
if (!file.exists("noticias_vectors.bin")) {
  model <- train_word2vec("noticias_w2v.txt", "noticias_vectors.bin",
          ...
          iter = 5, negative_samples = 50, min_count = 5) 
} else {
...
vector <-  model[["día"]] + plural_1
model %>% closest_to(vector, n = 3) %>% filter(word != "día")
```


```{r}
plural_1 <- model[["goles"]] - model[["gol"]]
vector <-  model[["juego"]] + plural_1
model %>% closest_to(vector, n = 4) %>% filter(word != "juego")
```
...
model %>% closest_to(vector, n = 3) %>% filter(word != "rey")
```
```{r}
fem_1 <- model[["presidenta"]] - model[["presidente"]]
vector <-  model[["tío"]] + fem_1
model %>% closest_to(vector, n = 3) %>% filter(word != "tío")
```

Por ejemplo, las palabras similares para lluvia y sol son:

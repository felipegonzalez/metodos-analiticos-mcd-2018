...
normalizar <- function(oracion){
  ...
  texto <- gsub("\\,", " _coma_ ", texto) 
  paste("_s_ _s_ ", texto)
}
corpus_mini <- c("Este es un ejemplo: el perro corre, el gato escapa. Este es un número 3.1416, otro número es 123.",
...
```{r}
set.seed(123)
periodico_m <- periodico_df %>% sample_n(1e4)
```
...
### Evaluación de probabilidad de frases

```{r}
n_gramas <- list(unigramas = mod_uni,
                 bigramas  = mod_bi,
                 trigramas = mod_tri)
log_prob <- function(textos, reg = 10000, n = 2){
  ...
  log_probs[is.na(log_probs)] <- log(1/reg)
...
- El modelo de bigramas da probabilidad 0 a las segunda frase (lo cual es factible), y también da una probabilidad relativamente a la tercera frase.
...
Como vimos arriba, el método de conteos es deficiente pues típicamente
pone probabilidad 0 a frases que pueden ocurrir en el lenguaje (es decir,
los datos del corpus típicamente son ralos en el espacio de posibles
frases del lenguaje). Una primera solución simple es suavizar los
conteos.

Para bigramas, usamos

$$P(w|z) = \frac{N(zw) + 1}{N(z)+V}$$

donde $V$ es el tamaño del vocabulario (el número de palabras únicas).




```{r}
V <- nrow(frec_unigramas)
V
textos <- c("xxay kacii ejemplo uus.",
            "Es este ejemplo un.",
           "Este es un ejemplo.",
           "Esta frase es normal.")
log_prob(textos, n = 1, a = 1, reg = V)
log_prob(textos, n = 2, a = 1, reg = V)
log_prob(textos, n = 3, a = 1, reg = V)
```

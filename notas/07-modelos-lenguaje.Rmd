...
```{r}
set.seed(123)
periodico_m <- periodico_df %>% sample_n(1e5)
```
...
### Cálculo de probabilidad de textos
...
n_gramas <- list(unigramas = mod_uni,
                 bigramas  = mod_bi,
                 trigramas = mod_tri)
log_prob <- function(textos, n_gramas, reg = 0, n = 2){
...
$</s>, <s>$). La **perplejidad** del modelo sobre este texto es igual a
...
textos <- c("xxay kacii ejemplo uus.",
           ...
           "Este es un ejemplo.",
           "Esta frase es normal.")
log_prob(textos, n = 1, reg = V)
log_prob(textos, n = 2, reg = V)
log_prob(textos, n = 3, reg = V)
```

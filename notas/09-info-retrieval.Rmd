...
periodico <- normalizar(periodico)
periodico[c(1,5,7)]
```
...
vec <- procesar_query("comida cocina", vocabulario_noticias)
...
```{r}
vec <- procesar_query("la comida y la cocina", vocabulario_noticias)
similitudes <- calc_sim(vec, td_noticias_tf)
recuperados <- periodico[order(similitudes[1,], decreasing = TRUE)[1:100]] 
head(recuperados, 10)
```

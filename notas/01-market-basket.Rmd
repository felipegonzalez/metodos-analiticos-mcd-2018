...
gigantesco. Asignaríamos incorrectamente 0 a la gran mayoría de canastas.
...
esto se debe a error de estimación. El coeficiente de variación del estimador $\hat{p}$
de la proporción en que ocurre un itemset es aproximadamente
$$\frac{1}{\sqrt(\hat{p}n)},$$
donde $n$ es el número de canastas. Vemos entonces que cuando $\hat{p}$ es chico esta variación
es muy grande
...
Muchas veces, esta información se organiza en términos de reglas de asociación. Un ejemplo de una regla de asociación es:
...
$lhs.support\times confidence = support$. Podemos entonces trabajar con
el par de métricas que nos convenga más.
...
```{block2, type='resumen'}
Los itemsets frecuentes $I$ que buscamos generalmente deben cumplir:
  
- El soporte de $I$ debe ser suficientemente alto (para que el itemset
sea relevante).
...
a_reglas <- apriori(lista_mb, parameter = pars)
df_reglas <- a_reglas %>% DATAFRAME
```

Esta elección de parámetros resulta en `r nrow(df_reglas)`, que
no es demasiado grande para poder examinar:

```{r}
DT::datatable(df_reglas %>%
  mutate_if(is.numeric, funs(round(., 2))))
```

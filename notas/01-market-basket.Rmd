...
total de itemsets de tamaño $k$ es $\binom{m}{k}$.
...
relativamente rápida todos los subconjuntos de tamaño $k$ (para $k=1,2,3,4$, por ejemplo). Por ejemplo, si una canasta tiene 10 artículos, hay $\binom{10}{3}$ canastas
de tamaño 3, $\binom{10}{4} = 210$. Calcular estos subconjuntos es relativamente rápido comparado con leer de disco una transacción.
...
**Observación**
- Este hecho a veces es un poco confuso (es monotonicidad decreciente), 
...
un artículo no frecuente. Así que para calcular pares:
  - Generamos los pares candidatos  a partir de los artículos frecuentes.
  - Recorremos el archivo de transacciones y solo contamos los pares posibles resultantes que 
  pertenencen al conjunto de pares candidatos.
...
2. Si en la segunda pasada no usáramos monotonicidad, tendríamos que mantener conteos de todos los posibles pares (que son del orden $m^2$, donde $m$ es el número de artículos). Mantener este conteo en memoria podría ser difícil. Sin embargo, el número de artículos frecuentes generalmente es considerablemente menor.
...
**Observaciones**: Hay distintas maneras de implementar este algoritmo:
...
- Otros algoritmos también son explicados en [@borgelt]. A priori es efectivo,
pero actualmente hay otros mejores.
...
sims <- lapply(1:10, function(i){
  ...
  df <- data.frame(table(size(ap_random)))
  df$rep <- i
  df
})
...
las canastas de *Groceries*, aunque también observamos que una buena proporción de canastas
de tamaño 2 puede explicarse por la co-ocurrencia de artículos que tienen frecuencia alta.

## Reglas de asociación

Aunque muchas veces lo único que nos interesa es la co-ocurrencia de artículos (por ejemplo,
para entender qué artículos se podrían ver afectados por acciones en otros artículos que
están en el mismo itemset frecuente), muchas veces nos interesa entender qué artículos
...
```{r, message = FALSE, results=FALSE}
pars <- list(supp = 0.02, confidence = 0.0, target='rules', 
             ext = TRUE, minlen = 2)
a_reglas <- apriori(lista_mb, parameter = pars)
```

```{r, message=FALSE, results=FALSE}
df_1 <- sort(a_reglas, by = 'confidence') %>%
        DATAFRAME 
```
...
### Valores de lift en el modelo simple
...
sims_reglas <- lapply(1:10, function(i){
  trans <- simular_transacciones(nItems = 169, 
                               ...
                               iprob = probs_items)
  ap_random <- apriori(trans, parameter = 
                       list(support=0.005, confidence = 0.0, 
...
```{r, warning=FALSE}
df_sims <- lapply(1:length(sims_reglas), function(i){ df <- sims_reglas[[i]] %>% DATAFRAME %>% mutate(rep=i)  }) %>% bind_rows
ggplot(df_sims, aes(x=support, y= lift)) + geom_point() + facet_wrap(~rep)
plotly_arules(sims_reglas[[2]], measure=c('support','lift'), shading = 'confidence')
```



```{r}
ap_reglas <- apriori(Groceries, parameter = 
                       list(support=0.005, confidence = 0.0, 
                            target='rules', ext = TRUE, minlen = 2),
                       control = list(verbose = FALSE))
  plotly_arules(ap_reglas, measure=c('lhs.support','confidence'), shading = 'confidence')
```

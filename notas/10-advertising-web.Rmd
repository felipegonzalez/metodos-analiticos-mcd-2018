...
puede dar el agoritmo de balance. El algoritmo miope se puede escribir como:

```{r}

greedy <- function(n_anunciantes, n_queries, lambda, presupuesto){
  balance <- rep(presupuesto, n_anunciantes)
  queries_oferta <- lapply(1:n_anunciantes, function(i) {
        num_queries <- min(rpois(1, lambda = lambda) + 1, n_queries)
        sample(1:n_queries, num_queries)})
  queries_activos <- unique(unlist(queries_oferta))
  ingreso <- 0
  iteraciones <- 0
  llega_query <- function(i){
    query <- sample(queries_activos, 1)
    # seleccionar los que tienen presupuesto
    candidatos_ind_pres <- balance > 0
    # seleccionar los que ofertaron por el query
    candidatos_ind_query <- sapply(queries_oferta, function(x){  query %in% x  })
    candidatos <- which(candidatos_ind_pres & candidatos_ind_query)
    if(sum(presupuesto) > 0){
      iteraciones <<- iteraciones + 1
      if(length(candidatos) > 0){
        ganador <-  first(candidatos)
...
```{r}  
balance <- function(n_anunciantes, n_queries, lambda, presupuesto){
  balance <- rep(presupuesto, n_anunciantes)
  queries_oferta <- lapply(1:n_anunciantes, function(i) {
        num_queries <- min(rpois(1, lambda = lambda) + 1, n_queries)
        sample(1:n_queries, num_queries)})
...
```{r}
#set.seed(343434)
adwords <- balance(n_anunciantes = 30, n_queries = 50, lambda = 1, presupuesto = 5)
miope <- balance(n_anunciantes = 30, n_queries = 50, lambda = 1, presupuesto = 5)

df_balance <- data_frame(n_iter = 1:150) %>% 
  mutate(datosl = map(n_iter, function(i){ 
           data_frame(
            ingreso = adwords(),
            balance = sum(environment(adwords)$balance))})) %>% unnest %>%
  mutate(algo = "balance")

df_greedy <- data_frame(n_iter = 1:150) %>% 
  mutate(datosl = map(n_iter, function(i){ 
           data_frame(
            ingreso = miope(),

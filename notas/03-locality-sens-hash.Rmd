...
combinaciones_50 <- params_umbral(50, 0.2, 0.6)
combinaciones_120 <- params_umbral(120, 0.2, 0.5)
...
textos <- articulos_df$categorias %>% lapply(function(x){
  paste(x, collapse = ' ')
}) %>% as.character
...
lo mismo (hacer LSH), bajo otras medidas de distancia. Veamos las características básicas de las funciones minhash:
 ...
 2. Podemos escoger varias funciones minhash $f_1,\ldots,f_k$ con la propiedad anterior, de manera independiente, de forma que es posible calcular
 la probabilidad de $f_1(x)=f_1(y)$ y $f_2(x)=f_2(y)$, y eventos
 combinados de este tipo.
 3. Tienen que ser relativamente fácil de calcular (comparado con calcular todos los posibles pares y sus distancias directamente).
 
 
## Funciones sensibles a la localidad

Sean $d_1,d_2>0$ dos valores (que interpretamos como distancias).

Una familia ${\cal F}$ es una familia $d_1,d_2,p_1,p_2$ sensible a localidad
cuando para cualquier par de elementos$x,y$,
...
ggplot(dat_g, aes(x=x, y=y)) + geom_point(size=0.5) +
  ...
  scale_y_continuous(breaks = c(1,1.5), labels = c('p_1','p_2')) +
...
si $d(x,y) = 1-sim(x,y) \leq 0.2$, entonces
...
azar de manera independiente, de forma
que $f(x)=f(y)$ si y sólo si $f_i(x)=f_i(y)$ para toda $i$. Esta construcción
corresponde a una banda de la técnica de LSH.
...
con similitudes en niveles bajos (por ejemplo, rechazar 0.1 y aceptar 0.3).
...
podemos construir una famlia que discrimine tan fuertemente como
queramos entre distancias menores a $d_1$ y distancias mayores a $d_2$.
...
Construimos ahora LSH para datos numéricos. Consideremos la tabla:

```{r, results = 'asis'}
df <- data_frame(x_1 = rnorm(100,0,1), x_2 = rnorm(100,2,1))
print(df)
```

Consideremos dos vectores, y supongamos que el ángulo entre ellos
es chico. Entonces, si escogemos un hiperplano al azar, los más
probable es que queden del mismo lado del hiperplano. Si los vectores
...
```{block2, type='resumen}
La familia descrita arriba es $(d_1,d_2, (180-d_1)/180, d_2/180)$
  sensible a la localidad para la distancia angular.
```

Supongamos que el ángulo entre $x$ y $y$ es $d$, es decir,
la distancia angular entre $x$ y $y$ es $d$. 

Consideramos el plano que pasa por el origen por $x$ y $y$. Si escogemos
el vector $v$ al azar, 

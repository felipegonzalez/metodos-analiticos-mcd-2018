...
- La probabilidad de
que observemos una cola de 0's de tamaño **al menos** $r$ es $2^{-m}$, para $m \geq 1$ 

- Supongamos
que tenemos una sucesión de $n$ candidatos del flujo distintos. La probabilidad de
que *ninguno* tenga una cola de tamaño al menos $m$ es igual a
\begin{equation}
(1-2^{-m})^{n}
(\#eq:probacola)
\end{equation}
...
si $n$ es más grande que $2^m$, y es cercana a 0 cuando $n$ es más chico que $2^m$.
...
- El error relativo teórico del algoritmo (con algunas mejoras) es de $1.04\sqrt{m}$, donde $m$ es el número de cubetas, así que más cubetas mejoran el desempeño.

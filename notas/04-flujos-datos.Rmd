...
### Ejemplos {#ejemplo}
```{r}
media.exp <- rep(NA, 5000)
media.exp[1] <- datos$obs[1]
c <- 0.2
for(i in 2:5000){
  media.exp[i] <- c*datos$obs[i]  + (1-c)*media.exp[i-1]
}
plot(datos$obs[51:5000], col='gray')
lines(media.exp, type="l", col='red', lwd=2)
```

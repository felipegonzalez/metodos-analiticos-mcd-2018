...
- Si es de esta película, actualizamos a $s_{t+1} = (1-c)*s_t + c*1$ (1 vista).
- Si es de otra película, actualizamos a $s_{t+1} = (1-c)*s_t + c*0= (1-c)*s_t$.

Una nueva película empieza con $s_0=0$, y en su primera vista, actualizamos a $s_t=c$.

Para mantener una lista de tamaño fijo, lo cual es importante si el número
...
Supongamos que queremos mantener una lista de tamaño fijo de los películas más vistas en netflix en cada momento (o los usuarios, posts, etc. más populares).

Si el número de películas es relativamente chico, podríamos llevar cuenta de todos los promedios exponenciales para todas las películas, y 
filtrar, por ejemplo, los que tengan promedio más alto.

Sin embargo, si la lista total es muy grande, y no queremos monitorear las películas poco populares, podemos hacer los siguiente cada vez que observamos un reporte

...
#numerar consecutivamente usuarios
...
Qusiéramos predecir el gusto usando estos dos vectores. Nuestras predicciones (considerando que u y v son vectores renglón) serían simplemente
...
funciona compartiendo información a lo largo de películas y usuarios. 
...
Finalmente, también podemos escribir este problema (recuérdese que $u_i$ y
...
```{r}
arrange(latentes_pelis, features_5) %>% 
  select(nombre, features_5, media_peli, num_calif_peli) %>% 
  filter(num_calif_peli > 500) %>% 
  head(100) 
arrange(latentes_pelis, features_5) %>% 
  select(nombre, features_5, media_peli, num_calif_peli) %>% 
...
## Tarea (para 26 de febrero)

- En la sección \@ref(simitems), mejora la función *ejemplo* para que en lugar del número
...
$$
L =  \sum_{(i,j) obs} (x_{ij} - a_i - b_j - \sum_{l=1}^k u_{i,l}v_{j,l})^2 +
\lambda \left ( \sum_i ||u_i||^2 + \sum_j ||v_j||^2 + \sum_i a_i^2 + \sum_i b_j^2 \right )
$$
...
```{block2, type = 'comentario'}
Sea $\gamma >0$ el tamaño de paso (tasa de aprendizaje) de descenso estocástico,
y $\lambda \geq 0$ una constante de regularización
Para cada evaluación $x_{i,j}$, la actualización de descenso está dada por,
en forma vectorial:
                                                                            
$$
u_i^{(n+1)} = u_i^{(n)} + \gamma \left ( e_{i,j} v_j^{(n)} - \lambda u_i^{(n)} \right )
$$
  
y

$$
v_j^{(n+1)} = v_j^{(n)} + \gamma \left ( e_{i,j} u_j^{(n)} - \lambda v_j^{(n)} \right )
$$


Para los sesgos, las actualizaciones son:

$$a_i^{(n+1)} = \gamma \left ( e_{i,j} - \lambda a_i^{(n)} \right )$$
  
$$a_i^{(n+1)} = \gamma \left ( e_{i,j} - \lambda b_j^{(n)} \right )$$
  
Inicializamos las matrices $U$ y $V$ con valores aleatorios chicos.
```

Puedes ver [este video](https://www.youtube.com/watch?v=GGWBMg0i9d4&index=56&list=PLLssT5z_DsK9JDLcT8T62VtzwyW9LNepV) de nuestro libro de texto
...
Podemos agregar tamibén dos usuarios nuevos con evaluaciones
...
nombres_evaluar <- data_frame(nombre = 
                    c('Ghost', 'Pretty Woman', 'Driving Miss Daisy',
                     ...
                     'The Terminator'))
evals_1 <- c(3, 2, 2, 4, 3, 4, 4, 2, 4, 5, 5, 5)
evals_2 <- 6 - evals_1
...
for(iter in 1:15){
  ...
  salida <- descenso_estocastico(i, j, y, U, V, mu, a, b, 
              gamma = 0.01, lambda_mat = 0.01, lambda_sesgos=0.5,
              n_iter = 1)
...
prefs <- data_frame(prefs = prefs_1, peli_id = 1:17770) %>% 
  ...
  arrange(desc(prefs)) %>% 
  filter(num_calif_peli > 500) %>%
  select(nombre, prefs) %>% print(n=20)
```
...
muestra_graf <- V_df %>%
  ...
  sample_n(120)
ggplot(muestra_graf,
  #filter(V_df, num_calif_peli > 5000, abs(V1^2+V2^2)> 0.5), 
       aes(x = V1, y= V2, label=nombre, colour=media_peli,3))+
           #colour = factor(ntile(V3,3)))) + 
  geom_point() + geom_text_repel(size=3.5, segment.alpha = 0.3)
```
...
ggplot(muestra_graf,
  #filter(V_df, num_calif_peli > 5000, abs(V1^2+V2^2)> 0.5), 
       aes(x = V3, y= V4, label=nombre, colour=media_peli,3))+
           #colour = factor(ntile(V3,3)))) + 
  geom_point() + geom_text_repel(size=3.5, segment.alpha = 0.3)
```
...
Esta sección está basada en [@recomendacion-implicito].
...
$i$ y el artículo $j$. Ponemos $r_{i,j}=0$ cuando no ha habido interacción
entre este usuario y el artículo. 


La diferencia importante con los ratings explícitos es que los datos
implícitos son menos informativos que los explícitos:
...
- Sin embargo, estamos seguros de que niveles altos de interacción (vio muchas veces una canción, etc.), es indicación de preferencia alta.
...
Una solución propuesta en [@recomendacion-implicito] (e implementada en 
...
Y la función objetivo se define como
...
los factores, indicando que valores de $r_{ij}$ son más informativos y
deben contribuir más en el ajuste. Adicionalmente, los valores
...
en el caso explícito. La diferencia es que algunas observaciones
que son 0 para un usuario pueden deberse a diversas razones, y no 
significa que ese item no le gustaría al usuario. Medidas
como RECM o MAD no son tan apropiadas para este problema. 

Una alternativa es, para cada usuario $i$ ordenar los artículos de 
mayor a menor valor de $\hat{p}_{ij}} = u_iv_j^t$ , 
seleccionar una lista de los mejores $K$ artículos
(canciones, pellículas), y calcular

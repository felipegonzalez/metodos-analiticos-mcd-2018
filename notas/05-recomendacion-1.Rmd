...
set.seed(2805)
#inicialización
U_0 <- matrix(rnorm(10 * max(i), 0, 0.01), ncol = 10)
V_0 <- matrix(rnorm(10 * 17770, 0, 0.01), ncol = 10)
U <- U_0
V <- V_0
#a <- rep(0, dim(X)[1])
#b <- rep(0, dim(X)[2])
...
a[is.nan(a)] <- 0
b <- colSums(X)/colSums(X>0)
print(sqrt(calc_error(i, j, y, U, V, a, b)))

for(iter in 1:10){
  # nota: la función descenso_gradiente (C++) no hace copias de U,V,a,b
  # cambia in-place estos parámetros
  out <- descenso_estocastico(i, j, y, U, V, a, b, 
              gamma = 0.01, lambda = 0.01, n_iter = 1)
  error_entrena <- sqrt(calc_error(i, j, y, U, V, a, b))
  error_valida <- sqrt(calc_error(i_val, j_val, y_val, U, V, a, b))
  sprintf("Error entrena: %.3f, Error valida: %.3f", 

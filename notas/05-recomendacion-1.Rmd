...
estocástica, puedes ver por ejemplo, la [notas de aprendizaje 
...
El resto de las derivadas son iguales a 0 (para otros $u_s^{(m)}$, 
$v_t^{(m)}$). Esto nos da la dirección del gradiente en cada paso del descenso estocástico. 

```{block2, type = 'comentario'}
Sea $\gamma >0$ el tamaño de paso de descenso estocástico.
Para cada evaluación $x_{i,j}$, la actualización de descenso está dada por (en
 forma vectorial):
...
Podemos implementar en C++ para tener mejor desempeño.


```{r}
library(Rcpp)
Rcpp::sourceCpp('../src/factorizacion_mat/descenso_mat.cpp')
...
set.seed(2805)
#inicialización
U_0 <- matrix(rnorm(5*dim(X)[1],0,0.01), ncol=5, nrow=dim(X)[1])
V_0 <- matrix(rnorm(5*dim(X)[2],0,0.01), ncol=5, nrow=dim(X)[2])
U <- U_0
V <- V_0
#a <- rep(0, dim(X)[1])
#b <- rep(0, dim(X)[2])
...
for(iter in 1:10){
out <- descenso_gradiente(i, j, y, U, V, a, b, 0.004, 0.01)
print(sqrt(calc_error(i,j,y, out$U, out$V, out$a, out$b)))
U <- out$U
V <- out$V
a <- out$a
b <- out$b

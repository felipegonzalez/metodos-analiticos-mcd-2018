...
Este método está implementado en [spark](https://spark.apache.org/docs/2.1.0/mllib-collaborative-filtering.html). La implementación está basada en [@alsreg].
...
```{r}
rm(list = c('muestra_nf', 'dat_entrena_2'))
library(sparklyr)
...
Verifiquemos el desempeño del modelo con la muestra de validación
...
Y redujimos considerablemente el error del modelo base.


```{r}
class(modelo$user_factors)
class(modelo$item_factors)
...
```{r}
arrange(latentes_pelis, features_1) %>% 
  select(nombre, features_1, media_peli, num_calif_peli) %>% 
  head(100) 
arrange(latentes_pelis, features_1) %>% 
  select(nombre, features_1, media_peli, num_calif_peli) %>% 
  tail(100)
...
La tercera dimensión captura el nivel general de gusto por la película.
Nótese que
...
ggplot(sample_n(latentes_pelis, 500), aes(x = features_2, 
                           y = media_peli, 
                           colour = log10(num_calif_peli))) +
  geom_point() + facet_wrap(~num_grupo) +

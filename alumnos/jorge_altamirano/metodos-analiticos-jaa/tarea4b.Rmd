...
```{r}
diccionario <- read.csv("datos/es_dic.txt", 
...
Revisamos si la palabra 'chilaquiles' esta en el diccionario
```{r}
bloom_filter$in_filter('chilaquiles')
```
...
Vemos si el diccionario nos recomienda alguna palabra similar
```{r}
generar_dist_1('chilaquiles') %>% keep(bloom_filter$in_filter)
```

Agregamos la palabra al diccionario.
```{r}
bloom_filter$add("chilaquiles")
```

Vemos si escribiendo la palabra 'chilaquilos', nos recomienda la palabra chilaquiles que ya es parte del diccionario.
```{r}
generar_dist_1('chilaquilos') %>% keep(bloom_filter$in_filter)
```

Vemos si escribiendo la palabra 'chilakilos', nos recomienda la palabra chilaquiles que ya es parte del diccionario.
```{r}
generar_dist_1('chilakilos') %>% keep(bloom_filter$in_filter)
```

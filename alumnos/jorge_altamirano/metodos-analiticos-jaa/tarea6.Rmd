...
############## 2 sub
# 2 usuarios más
i_usuario_1 <- rep(100001, 14)
i_usuario_2 <- rep(100002, 14)
i_usuario_3 <- rep(100003, 14)
i_usuario_4 <- rep(100004, 14)
...
nombres_evaluar <- data_frame(nombre = 
                    c('Ghost', 'Pretty Woman', 'Driving Miss Daisy',
                     ...
                     'The Shining'))
dim(nombres_evaluar)
evals_1 <- c(1, 2, 2, 4, 3, 4, 4, 2, 4, 5, 5, 5, 5, 5)
evals_2 <- 6 - evals_1
...
### Lambda Mat 0.1, Lambda Sesgos 1
...
for(iter in 1:15){
  ...
  # cambia in-place estos parámetros
  salida <- descenso_estocastico(i, j, y, U, V, mu, a, b, 
              gamma = 0.003, lambda_mat = 0.1, lambda_sesgos = 1,
              n_iter = 1, num_peli, num_usu)
  error_entrena <- sqrt(calc_error(i, j, y, U, V, mu, a, b))
  error_valida <- sqrt(calc_error(i_val, j_val, y_val, U, V, mu, a, b))
  sprintf("Iteración: %i, Error entrena: %.4f, Error valida: %.4f", 
...
### Lambda Mat 0.1, Lambda Sesgos 0.1
...
for(iter in 1:15){
  ...
  # cambia in-place estos parámetros
  salida <- descenso_estocastico(i, j, y, U, V, mu, a, b, 
              gamma = 0.003, lambda_mat = 0.1, lambda_sesgos = 0.1,
              n_iter = 1, num_peli, num_usu)
  error_entrena <- sqrt(calc_error(i, j, y, U, V, mu, a, b))
  error_valida <- sqrt(calc_error(i_val, j_val, y_val, U, V, mu, a, b))
  sprintf("Iteración: %i, Error entrena: %.4f, Error valida: %.4f", 
...
### Lambda Mat 0.8, Lambda Sesgos 0.001
...
for(iter in 1:15){
  ...
  # cambia in-place estos parámetros
  salida <- descenso_estocastico(i, j, y, U, V, mu, a, b, 
              gamma = 0.003, lambda_mat = 0.8, lambda_sesgos = 0.001,
              n_iter = 1, num_peli, num_usu)
  error_entrena <- sqrt(calc_error(i, j, y, U, V, mu, a, b))
  error_valida <- sqrt(calc_error(i_val, j_val, y_val, U, V, mu, a, b))
  sprintf("Iteración: %i, Error entrena: %.4f, Error valida: %.4f", 
